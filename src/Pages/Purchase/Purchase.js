...
const Purchase = () => {
	...
	useEffect(() => {
		fetch(`https://damp-wave-89480.herokuapp.com/service/${id}`)
			.then((res) => res.json())
	...
	const handleSubmit = (event) => {
		...
		const btnDisabled = totalQuantity < previousQuantity || totalQuantity > stockQuantity;
		...
		if (totalQuantity < previousQuantity) {
			toast(`Please increase your Quantity! Minimum order ${previousQuantity} `);
			return;
		} else if (totalQuantity > stockQuantity) {
			toast(`Sorry! We don't have ${stockQuantity} ${name} in our stock. Please reduce the quantity!`);
			return;
		}
		...
		fetch("https://damp-wave-89480.herokuapp.com/purchase", {
			method: "POST",
			headers: {
			...
			.then((data) => {
				if (data.success) {
					toast.success(
						`successfully purchased ${data.purchase.toolsName} tools.`
					);
				} else {
					toast.error(
						`Sorry!! You already purchased ${data.purchase.toolsName} tools.`
					);
				}
			});
	};
					...
					<div className="card border flex-shrink-0 w-full h-[750px] max-w-sm shadow-2xl bg-base-100">
						<div className="card-body ">
							...
							<div className="grid grid-cols-2 gap-10">
								<div>
									<p className=" bg-primary text-white rounded-t-lg py-2">
										Minimum
									</p>
									<p className="border-2 border-primary rounded-b-lg font-bold text-2xl text-red-500">
										{minQuantity}
									</p>
								</div>
								<div>
							...
							<form onSubmit={handleSubmit}>
								<div className="grid grid-cols-2 gap-10">
									<div>
										...
										<input
											...
											name="quantity"
											placeholder="Qnty..."
											className="input input-bordered font-bold text-xl input-primary w-full max-w-xs"
										/>
									</div>
									<div>
										<label className="label">
										...
										<input
											...
											name="price"
											value={prices}
											className="input input-bordered font-bold input-primary text-xl w-full max-w-xs"
										/>
									</div>
								</div>
								<label className="label">
								...
								<input
									...
									className="font-bold input input-bordered input-primary w-full max-w-xs"
								/>
								<label className="label">
									{" "}
								...
								<label className="label">
									{" "}
									<span className="label-text font-bold mt-2">Phone</span>
								</label>
								<input
									type="text"
									name="phone"
									...
									required
									className="input font-bold input-bordered input-primary w-full max-w-xs"
								/>
								<label className="label">
									{" "}
									<span className="label-text font-bold mt-2">Address</span>
								</label>
								<input
									type="text"
									name="address"
