...
const MyOrders = () => {
	...
	useEffect(()=>{
		fetch('https://damp-wave-89480.herokuapp.com/payment')
		.then(res => res.json())
	...
	useEffect(() => {
		if (user) {
			fetch(`https://damp-wave-89480.herokuapp.com/purchase?userEmail=${user.email}`, {
				method: "GET",
				headers: {
	...
	const handleDelete = (id) => {
		...
		if(proceed){
			const url = `https://damp-wave-89480.herokuapp.com/purchase/${id}`;
			fetch(url, {
				method: 'DELETE'
			})
			.then(res => res.json())
